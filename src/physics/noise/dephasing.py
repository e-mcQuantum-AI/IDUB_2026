"""Phase damping (dephasing) quantum channel.

This module implements a bosonic dephasing channel using the Lindblad
master equation formalism.
"""

import numpy as np
from qutip import destroy, mesolve, Qobj
from .base import QuantumChannel


class DephasingChannel(QuantumChannel):
    """Phase damping (dephasing) channel.

    Models the destruction of phase coherence (decoherence) in a bosonic mode
    without changing the photon number populations, described by a Lindblad
    master equation with a single collapse operator proportional to the
    number operator.

    The evolution of the density matrix ρ is given by:

        dρ/dt = γ (n̂ ρ n̂ - 1/2 {n̂², ρ})

    where n̂ = a† a is the photon number operator and γ is the dephasing rate.

    Physically, this corresponds to elastic scattering processes or
    fluctuations in the environment's refractive index, causing the
    off-diagonal elements of ρ in the Fock basis to decay exponentially.

    In this implementation, the channel is obtained by solving the
    master equation over a fixed time interval t ∈ [0, 1], so γ
    effectively sets the strength of the dephasing.

    Args:
        cutoff (int): Hilbert space dimension.
        gamma (float): Dephasing rate (decoherence strength).

    Raises:
        ValueError: If gamma is negative.
    """

    def __init__(self, cutoff: int, gamma: float) -> None:
        """Initialize the dephasing channel.

        Args:
            cutoff (int): Hilbert space dimension.
            gamma (float): Dephasing rate.

        Raises:
            ValueError: If gamma is negative.
        """
        if gamma < 0:
            raise ValueError(f"gamma must be non-negative, got {gamma}")

        self.cutoff = cutoff
        self.gamma = gamma

    def apply(self, rho: Qobj) -> Qobj:
        """Apply the dephasing channel to a density matrix.

        Evolves the input state according to the Lindblad master equation
        with a single collapse operator √γ · n̂ over the time interval [0, 1].

        Args:
            rho (Qobj): Input density matrix.

        Returns:
            Qobj: Output density matrix after evolution.

        Raises:
            ValueError: If the input is not an operator (density matrix).
        """
        if not rho.isoper:
            raise ValueError("Expected a density matrix (Qobj operator).")

        a: Qobj = destroy(self.cutoff)
        n_hat: Qobj = a.dag() * a

        result = mesolve(
            H=0 * n_hat,
            rho0=rho,
            tlist=[0, 1],
            c_ops=[np.sqrt(self.gamma) * n_hat],
        )

        return result.states[-1]
